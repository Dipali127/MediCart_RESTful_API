...
const placeOrder = async function (req, res) {
  try {
    ...
    await orderModel.findByIdAndUpdate(createOrder._id, {
      $set: { razorpayOrderId: razorpayOrder.id },
    });
    return res.status(200).send({razorpayOrderId: razorpayOrder.id,amount: amountInPaise, buyerId: createOrder.buyerId});
  } catch (error) {
