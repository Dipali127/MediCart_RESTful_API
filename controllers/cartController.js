    ...
    try {
        ...
        if (medicineExistinCart === -1) {
            const addMedicineinCart = await cartModel.findOneAndUpdate({ buyerId: buyerId },
                {
                    $push: { items: { medicineId: medicineId, quantity: 1 } }, $inc: { totalPrice: +isexistMedicine.price }
                }, { new: true }).populate('items.medicineId')
        ...
        if (medicineExistinCart > -1) {
            const incrementMedicineQuantity = await cartModel.findOneAndUpdate({ buyerId: buyerId, "items.medicineId": medicineId },
                { $inc: { "items.$.quantity": 1 }, $set: { totalPrice: userCart.totalPrice + isexistMedicine.price } }, { new: true }
...
module.exports = { addMedicineTocart, viewCart, deleteMedicinefromCart };